...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildContent(item) {
    ...
    // Presentation links (exclude for published papers)
    let presLinks = [];
    if (item.presentations && item.presentations.length > 0 && item.type !== 'Paper') {
      item.presentations.forEach(pres => {
        if (pres.url) {
          presLinks.push(`<a href="${pres.url}" target="_blank" rel="noopener"
    ...
    const openLink = links.concat(presLinks).join('<span style="color:var(--text-light);font-size:.74rem;margin:0 .4rem;"> • </span>');
